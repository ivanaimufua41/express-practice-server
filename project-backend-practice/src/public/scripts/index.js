...
function displayAllMessages() {
    httpGet('/api/messages/all')
        ...
        .then((response) => {
            ...
            const allMessageAnchor = document.getElementById('all-msg-anchor');
            allMessageAnchor.innerHTML += getMessageDisplayEle(allMessages);
        });
}

function getMessageDisplayEle(messages) {
    return messages.map((msg) => {
        `<div class="normal-view">
            <ul>
                <li>${msg.id}</li>
                <li>${msg.userId}</li>
                <li><img src=${msg.imagePath} />}</li>
                <li>${msg.message}</li>
            </ul>
        </div>`
    });
